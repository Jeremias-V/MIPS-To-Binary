...
loop:
    ...
    bne     $s1, $s3, loop  # while i != size, loop = (0x400048-0x40005C)/4 = -5        0x0040005C
    jr      $ra             # return to main after finishing the iteration              0x00400060

main:

    lui     $s0, 0x1000     # loads upper part of first memory position in $s0          0x00400064
    ori     $s0, 0x0000     # loads lower part of first memory position  in $s0         0x00400068
    ...
    addi    $s4, $0, 1      # Default number to fill array                              0x00400078
    j       fill            # go to fill = 0x00100009 * 4                               0x0040007C
